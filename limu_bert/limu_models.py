...
from LittleBeatsPrelim.limu_bert.limu_utils import split_last, merge_last
...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def make_w2v_compatibility_layers(self, w2v_hidden_size):
        self.proj_v_L_scaledown = nn.Linear(w2v_hidden_size, self.cfg.hidden)
        self.proj_v_C_scaledown = nn.Conv1d(1499, 120, 1)
        self.proj_k_L_scaledown = nn.Linear(w2v_hidden_size, self.cfg.hidden)
        self.proj_k_C_scaledown = nn.Conv1d(1499, 120, 1)
        self.flatten = nn.Flatten()
        self.A = nn.Conv1d(2, 1, 1)

    def forward(self, x, key_value_states=None):
        """
        x, q(query), k(key), v(value) : (B(batch_size), S(seq_len), D(dim))
        * split D(dim) into (H(n_heads), W(width of head)) ; D = H * W
        """
        if(key_value_states is not None):
            key_states = self.proj_k_L_scaledown(key_value_states[0])
            key_states = self.proj_k_C_scaledown(key_states)
            value_states = self.proj_v_L_scaledown(key_value_states[1])
            value_states = self.proj_v_C_scaledown(value_states)
            b, c, l = key_states.shape
            key_value_states = self.A(torch.cat(
                (self.flatten(key_states).unsqueeze(dim=1), self.flatten(value_states).unsqueeze(dim=1)),
                dim=1))
            key_value_states = key_value_states.reshape(b, c, l)
        else:
            key_value_states = x
        # (B, S, D) -proj-> (B, S, D) -split-> (B, S, H, W) -trans-> (B, H, S, W)
        q, k, v = self.proj_q(x), self.proj_k(key_value_states), self.proj_v(key_value_states)
...
class LBLIMUBertModel4Pretrain(nn.Module):
    def __init__(self, cfg, output_embed=False):
        super().__init__()
        self.embed = Embeddings(cfg)
        self.layers = [LBTransformer(cfg) for _ in range(cfg.n_layers)]
    ...
    def forward(self, input_seqs, masked_pos=None):
        h_masked = self.embed(input_seqs)
        for transformer_layer in self.layers:
            h_masked = transformer_layer(h_masked)
            # def create_custom_forward(module):
            #     def custom_forward(*inputs):
            #         return module(*inputs)
            #     return custom_forward
            #
            # h_masked = torch.utils.checkpoint.checkpoint(
