...
class Wav2Vec2LayerNormConvLayer(nn.Module):
    def __init__(self, config, layer_id=0):
        ...
        self.layer_norm1 = nn.LayerNorm(self.out_conv_dim, elementwise_affine=True)
        self.activation1 = ACT2FN[config.feat_extract_activation]

        self.layer_norm2 = nn.LayerNorm(self.out_conv_dim, elementwise_affine=True)
        self.activation2 = ACT2FN[config.feat_extract_activation]

        # self.softparam = SoftParamShare()
    ...
    def forward(self, hidden_states):
        ...
        if(len(hidden_states1.shape)==2):
            hidden_states1 = hidden_states1.unsqueeze(dim=1)
        hidden_states1 = self.conv1(hidden_states1)
        hidden_states1 = hidden_states1.transpose(-2, -1)
        ...
        if (len(hidden_states2.shape) == 2):
            hidden_states2 = hidden_states2.unsqueeze(dim=1)
        hidden_states2 = self.conv2(hidden_states2)
        hidden_states2 = hidden_states2.transpose(-2, -1)
        hidden_states2 = self.layer_norm2(hidden_states2)
        hidden_states2 = hidden_states2.transpose(-2, -1)
        hidden_states2 = self.activation2(hidden_states2)

        # hidden_states1, hidden_states2 = self.softparam(hidden_states1, hidden_states2)
        hidden_states = torch.cat((hidden_states1.unsqueeze(dim=1), hidden_states2.unsqueeze(dim=1)), dim=1)
        return hidden_states
...
class SelfDocLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.attn_layers1 = nn.ModuleList([Wav2Vec2Attention(
            embed_dim=config.hidden_size,
            num_heads=config.num_attention_heads,
            dropout=config.attention_dropout,
            is_decoder=False,) for _ in range(4)])
        self.attn_layers2 = nn.ModuleList([Wav2Vec2Attention(
            embed_dim=config.hidden_size,
            num_heads=config.num_attention_heads,
            dropout=config.attention_dropout,
            is_decoder=False, ) for _ in range(4)])
    ...
    def forward(
        ...
        hidden_states2 = self.layer_norm2(hidden_states2)
        hidden_states1, _, _ = self.attn_layers1[0](hidden_states1, attention_mask=attention_mask1, output_attentions=output_attentions, _key_states=hidden_states2, _value_states=hidden_states1, _query_states=hidden_states1)
        hidden_states2, _, _ = self.attn_layers2[0](hidden_states2, attention_mask=attention_mask2, output_attentions=output_attentions, _key_states=hidden_states1, _value_states=hidden_states2, _query_states=hidden_states2)
        hidden_states1, _, _ = self.attn_layers1[1](hidden_states1, attention_mask=attention_mask1, output_attentions=output_attentions)
        hidden_states2, _, _ = self.attn_layers2[1](hidden_states2, attention_mask=attention_mask2, output_attentions=output_attentions)
        hidden_states1, _, _ = self.attn_layers1[2](hidden_states1, attention_mask=attention_mask1, output_attentions=output_attentions, _key_states=hidden_states1, _value_states=hidden_states2, _query_states=hidden_states1)
        hidden_states2, _, _ = self.attn_layers2[2](hidden_states2, attention_mask=attention_mask2, output_attentions=output_attentions, _key_states=hidden_states2, _value_states=hidden_states1, _query_states=hidden_states2)
        hidden_states1, attn_weights1, _ = self.attn_layers1[3](hidden_states1, attention_mask=attention_mask1, output_attentions=output_attentions)
        hidden_states2, attn_weights2, _ = self.attn_layers2[3](hidden_states2, attention_mask=attention_mask2, output_attentions=output_attentions)
        ...
        hidden_states = torch.cat((hidden_states1.unsqueeze(dim=1), hidden_states2.unsqueeze(dim=1)), dim=1)
        if attn_weights1 is not None and attn_weights2 is not None:
            attn_weights = torch.cat((attn_weights1.unsqueeze(dim=1), attn_weights2.unsqueeze(dim=1)), dim=1)
...
#         is_self_doc = (_key_states is not None and _value_states is not None and _query_states is not None)
#
...
# Copied from above with cross attention applied on key and query
class Wav2Vec2Attention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
...
class Wav2Vec2EncoderLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.attention1 = Wav2Vec2Attention(
            embed_dim=config.hidden_size,
            num_heads=config.num_attention_heads,
            dropout=config.attention_dropout,
            is_decoder=False,
        )
        self.attention2 = Wav2Vec2Attention(
            embed_dim=config.hidden_size,
            num_heads=config.num_attention_heads,
            dropout=config.attention_dropout,
            is_decoder=False,
    ...
    def forward(self, hidden_states, attention_mask=None, output_attentions=False):
        attn_residual1 = hidden_states[:,0,:]
        attn_residual2 = hidden_states[:,1,:]
        hidden_states1, attn_weights1, _ = self.attention1(
            hidden_states[:,0,:], key_value_states=hidden_states[:,1,:], attention_mask=attention_mask, output_attentions=output_attentions
        )
        hidden_states1 = self.dropout(hidden_states1)
        hidden_states1 = attn_residual1 + hidden_states1

        hidden_states1 = self.layer_norm1(hidden_states1)
        hidden_states1 = hidden_states1 + self.feed_forward1(hidden_states1)
        hidden_states1 = self.final_layer_norm1(hidden_states1)

        hidden_states2, attn_weights2, _ = self.attention2(
            hidden_states[:, 1, :], key_value_states=hidden_states[:, 0, :], attention_mask=attention_mask,
        ...
        hidden_states2 = self.layer_norm2(hidden_states2)
        hidden_states2 = hidden_states2 + self.feed_forward2(hidden_states2)
        hidden_states2 = self.final_layer_norm2(hidden_states2)
...
class Wav2Vec2EncoderLayerStableLayerNorm(nn.Module):
    def __init__(self, config, cross_attention=False, limu_cfg=None, limu_transformer=None):
        super().__init__()
        self.config = config
        self.attention1 = Wav2Vec2Attention(
            embed_dim=config.hidden_size,
        ...
        if(limu_cfg is not None):
            self.limu_cfg = limu_cfg
            self.attn = copy.deepcopy(limu_transformer.attn)
            self.attn.make_w2v_compatibility_layers(config.hidden_size)
            self.proj = copy.deepcopy(limu_transformer.proj)
            self.norm1 = copy.deepcopy(limu_transformer.norm1)
            self.pwff = copy.deepcopy(limu_transformer.pwff)
            self.norm2 = copy.deepcopy(limu_transformer.norm2)

    def forward(
        self,
        hidden_states: torch.Tensor,
        limu_hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        output_attentions: bool = False,
    ):
        hidden_states1 = hidden_states[:,0,:,:]
        ...
        if(self.cross_attention):
            hidden_states1, attn_weights1, _ = self.attention1(
                hidden_states1, key_value_states=[hidden_states2, limu_hidden_states], attention_mask=attention_mask1, output_attentions=output_attentions, mode='cross'
            )
        else:
            hidden_states1, attn_weights1, _ = self.attention1(
                hidden_states1, attention_mask=attention_mask1, output_attentions=output_attentions, mode='self'
            )
        ...
        if(self.cross_attention):
            hidden_states2, attn_weights2, _ = self.attention2(
                hidden_states2, key_value_states=[hidden_states1, limu_hidden_states], attention_mask=attention_mask2, output_attentions=output_attentions, mode='cross'
            )
        else:
            hidden_states2, attn_weights2, _ = self.attention2(
                hidden_states2, attention_mask=attention_mask2, output_attentions=output_attentions, mode='self'
            )
        hidden_states2 = self.dropout(hidden_states2)
        hidden_states2 = attn_residual2 + hidden_states2
        hidden_states2 = hidden_states2 + self.feed_forward2(self.final_layer_norm2(hidden_states2))
        ...
        if self.config.mode == 'triple':
            if(self.cross_attention):
                ...
                limu_hidden_states = self.attn(limu_hidden_states)
            limu_hidden_states = self.proj(limu_hidden_states)
            limu_hidden_states = self.norm1(limu_hidden_states)
            limu_hidden_states = self.pwff(limu_hidden_states)
            limu_hidden_states = self.norm2(limu_hidden_states)
            # limu_hidden_states = limu_hidden_states.mean(dim=1)

        hidden_states = torch.cat((hidden_states1.unsqueeze(dim=1), hidden_states2.unsqueeze(dim=1)), dim=1)
        if attn_weights1 is not None and attn_weights2 is not None:
            attn_weights = torch.cat((attn_weights1.unsqueeze(dim=1), attn_weights2.unsqueeze(dim=1)), dim=1)

        outputs = (hidden_states, limu_hidden_states)
...
class Wav2Vec2Encoder(nn.Module):
    ...
    def forward(
        ...
        position_embeddings1 = self.pos_conv_embed1(hidden_states[:,0,:])
        hidden_states1 = hidden_states[:,0,:] + position_embeddings1
        hidden_states1 = self.layer_norm1(hidden_states1)
        hidden_states1 = self.dropout(hidden_states1)

        position_embeddings2 = self.pos_conv_embed2(hidden_states[:,1,:])
        hidden_states2 = hidden_states[:,1,:] + position_embeddings2
        hidden_states2 = self.layer_norm2(hidden_states2)
        hidden_states2 = self.dropout(hidden_states2)
        ...
        for layer in self.layers:
            ...
            if not skip_the_layer or deepspeed_zero3_is_enabled:
                ...
                hidden_states = torch.cat((layer_outputs[0][:,0,:], layer_outputs[0][:,1,:]), dim=1)
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not skip_the_layer or deepspeed_zero3_is_enabled:
                ...
                limu_hidden_states = layer_outputs[1]
...
"""
class SoftParamShare(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv_1 = nn.Conv1d(2, 1, 1)
        self.conv_2 = nn.Conv1d(2, 1, 1)

    def forward(self, x1, x2=None):
        if x2 is None:
            orig_shape = x1.shape
            x1 = x1.reshape(orig_shape[0], orig_shape[1], -1)
            y1 = self.conv_1(x1)
            y2 = self.conv_2(x1)
            y = torch.cat((y1, y2), dim=1).reshape(orig_shape)
            return y
        if x2 is not None:
            orig_shape = x1.shape
            B = orig_shape[0]
...
class Wav2Vec2Model(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        extract_features = self.feature_extractor(input_values[:,:2,:])
        extract_features1 = extract_features[:, 0, :].transpose(1, 2)
        extract_features2 = extract_features[:, 1, :].transpose(1, 2)

        if attention_mask is not None:
            '''
                not sure if this is right, but probably wont use it for now
            '''
            # compute reduced attention_mask corresponding to feature vectors
            attention_mask1 = self._get_feature_vector_attention_mask(
                extract_features1.shape[1], attention_mask[:,0].unsqueeze(dim=1), add_adapter=False
            )
            attention_mask2 = self._get_feature_vector_attention_mask(
                extract_features2.shape[1], attention_mask[:,1].unsqueeze(dim=1), add_adapter=False
            )

        hidden_states1, extract_features1 = self.feature_projection1(extract_features1)
        hidden_states1 = self._mask_hidden_states(
            hidden_states1, mask_time_indices=mask_time_indices, attention_mask=attention_mask1
        )

        hidden_states2, extract_features2 = self.feature_projection2(extract_features2)
        hidden_states2 = self._mask_hidden_states(
        ...
        if not return_dict:
            return (hidden_states, torch.cat((extract_features1.unsqueeze(dim=1), extract_features2.unsqueeze(dim=1)), dim=1)) + encoder_outputs[1:]
